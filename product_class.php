...
class Products {
    ...
    {
        $category = $this->con->escape_string($_POST['category']);
        echo $category;
        ...
        $query = "INSERT INTO `products` (`model`,`title`,`price`,`display`,`hardware`, `camera`, `battery`, `qty`, `images`, `category_id`) VALUES ('$model','$title','$price','$display','$hardware','$camera','$battery','$qty','$filename', '$category')";
    ...
    public function updateRecord($postData)
    {
        ...
        $category_id = $this->con->real_escape_string($_POST['category']);
        $id = $this->con->real_escape_string($_POST['id']);
        if (!empty($id) && !empty($postData)) {
            $query = "UPDATE products SET category_id = '$category_id', model = '$model', title = '$title', display = '$display' , qty = '$qty', battery = '$battery', hardware = '$hardware', price = '$price', camera = '$camera' WHERE id = '$id'";
            $sql = $this->con->query($query);
            if ($sql == true) {
    ...
    public function updateCategory($postData){
        ...
        $id = $this->con->real_escape_string($_POST['id']);
        $catId = $this->con->real_escape_string($_POST['catId']);
        if (!empty($id) && !empty($postData)){
            $query = "UPDATE products SET category_id = '$category' WHERE id = '$id'";
            $sql = $this->con->query($query);
            if ($sql == true){
                header("Location:edit_product_categories.php?catId=$catId");
            } else {
                echo "Update Failed";
            }
        }
       ...
       public function delete($id)
       {
        ...
        public function displayProductByCat($catId){
